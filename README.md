...
├── package.json         # NPM конфигурация и зависимости Electron
├── pwa/
│   ├── manifest.json    # PWA манифест
│   └── sw.js            # Service Worker для оффлайн-режима
│
├── electron/            # 🖥️ Electron приложение (NEW!)
│   ├── main.js          # Главный процесс Electron
│   ├── preload.js       # Безопасный API для renderer
│   └── wallpaper-api.js # Windows API для установки обоев
│
...
- ✅ **Установка обоев** — кнопка в FAB-меню для Windows
...
| Веществ с детальной информацией | 10+ (растёт!) |
...
Проект теперь можно запустить как десктопное приложение с функцией установки обоев!

#### Что нового:
- ✨ **Scroll-Collapse система** — таблица сжимается при скролле
- 🖼️ **Установка обоев** — кнопка в FAB-меню (только в Electron)
- 🪟 **Windows API интеграция** — автоматическая установка обоев через PowerShell
- 📦 **Portable версия** — не требует установки
...
После сборки `.exe` файлы будут в папке `dist/`:
- `ChemicalAssistant-Portable-1.0.0.exe` — портативная версия (просто запустить)
- `Chemical Assistant Wallpaper Setup 1.0.0.exe` — инсталлятор с меню Пуск и ярлыками

#### Как использовать функцию обоев:
1. Запустите Electron приложение
2. Настройте таблицу как хотите (тема, масштаб, открытые элементы)
3. Откройте FAB-меню (круглая кнопка справа снизу)
4. Нажмите "Установить обои"
5. Готово! Текущий вид таблицы станет обоями вашего рабочего стола
...
### Документация:
- [`AI_CONTEXT.md`](AI_CONTEXT.md) — технические ограничения и структура
- [`TODO.md`](TODO.md) — список задач и баги
