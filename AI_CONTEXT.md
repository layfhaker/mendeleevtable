...
│   ├── particles.js                 # Canvas-анимация фона и 3D атомов
│   │
│   ├── solubility/                  # Модули таблицы растворимости
│   │   ├── solubility-data.js       # Данные: катионы, анионы, матрица
│   │   ├── solubility-colors.js     # 100+ цветов веществ (substanceColors)
│   │   ├── solubility-table.js      # Рендеринг таблицы
│   │   ├── solubility-filters.js    # Фильтрация и выделение
│   │   ├── solubility-search.js     # Поиск веществ
│   │   └── solubility-modal.js      # Открытие/закрытие модалки
│   │
...
**Итого:** 9 CSS файлов, 15 JS файлов (4 базовых + 6 solubility + 5 modules)
...
const elementsData = {
    "H": {
        ...
        extraAllotropes: { fullerene: {...} }
    },
    // ... ещё 39 элементов (до Zr)
};
```

### 2. solubilityData (js/solubility/solubility-data.js)
```javascript
...
### 3. substanceColors (js/solubility/solubility-colors.js)
```javascript
const substanceColors = {
    ...
    "K₂CrO₄": { color: "#FFD700", name: "Жёлтый раствор" },
    "K₂Cr₂O₇": { color: "#FF4500", name: "Оранжевый раствор" },
...
// Волна запускается от кнопки, элементы меняют цвет по мере прохождения
```
...
@media (max-width: 768px) and (orientation: landscape) { /* Горизонтальная ориентация */ }
...
- ✅ Детальная информация по 40 элементам (H–Zr)
...
### Известные баги:
- ⚠️ Дублирование элемента Og в HTML
- ⚠️ Опечатка "Руфений" вместо "Рутений"
